...
// In-memory challenge store for serverless resilience
global._otpChallenges = global._otpChallenges || new Map();
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // Check against registered administrators
    let user = null;
    try {
    ...
    try {
      ...
      if (process.env.DATABASE_URL && user.id) {
        ...
        await prisma.otpChallenge.create({
          data: { userId: user.id, otpHash: tokenHash, expiresAt },
        });
      }
    } catch (_) {}
